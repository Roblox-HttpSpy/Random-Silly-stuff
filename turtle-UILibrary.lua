--// i found this in "https://github.com/bloodball/-back-ups-for-libs/blob/main/turtle" but this seems just a backup
--// all credits goes to who ever made the original ui library,
--// i have Re-Modified it, made it more secure and modified some small things (thats it really)
--// i mostly gonna use this for my own personal projects but feel free to also use it
local env = getgenv() or _G
if env.VHJ1cmxlTGliTW9kZGVkQnlIdHRw then
    return
end
env.VHJ1cmxlTGliTW9kZGVkQnlIdHRw = true
...
TurtleUiLib.Name = "TurtleUiLib[Modified]"
HideUI(TurtleUiLib)

local xOffset = 20
local keybindConnection

local function library:Window(name) 
    ...
    Instance.new("UIDragDetector", UiWindow)
    ...
    local function functions:Toggle(text, on, callback)
        local callback = callback or function() end
    ...
    local function functions:Box(text, callback)
        local callback = callback or function() end
    ...
    local function functions:Slider(text, min, max, default, callback)
    ...
    local function functions:Dropdown(text, buttons, callback)
        ...
        local dropFunctions = {}
        local canvasSize = 0
        local function dropFunctions:Button(name)
            local name = name or ""
        ...
        local function dropFunctions:Remove(name)
            local foundIt
    ...
    local function functions:ColorPicker(name, default, callback)
        local callback = callback or function() end
	    ...
	    local colorFuncs = {}
        local function colorFuncs:UpdateColorPicker(color)
